...
// Campaign already created on the first (failed) run — reuse it instead of duplicating.
const EXISTING_CAMPAIGN_ID = "6986639072081";
...
// Per user's explicit instruction: allow Meta's Advantage+ Audience to expand
// delivery beyond the custom (retargeting) audience, rather than restricting to it.
...
Deno.serve(async (_req: Request) => {
  ...
  const results: Record<string, unknown> = { campaign: { id: EXISTING_CAMPAIGN_ID, reused: true } };

  const { data: acctRow } = await supabase.from("ad_accounts").select("id").eq("meta_ad_account_id", META_AD_ACCOUNT_ID).maybeSingle();
  let { data: campaignRow } = await supabase.from("campaigns").select().eq("meta_campaign_id", EXISTING_CAMPAIGN_ID).maybeSingle();
  if (!campaignRow) {
    const { data: inserted } = await supabase.from("campaigns").insert({
      meta_campaign_id: EXISTING_CAMPAIGN_ID, campaign_name: "LUNESSA RETARGETING CAMPAIGN", campaign_type: "testing",
  ...
  for (const plan of adSetPlan) {
    const adset = await createAdSet(EXISTING_CAMPAIGN_ID, plan.name, plan.genders);
    ...
    const { data: adsetRow } = await supabase.from("ad_sets").insert({
      ...
      targeting_type: "retargeting", budget_naira: 3000, status: "paused", age_min: 25, age_max: 65,
