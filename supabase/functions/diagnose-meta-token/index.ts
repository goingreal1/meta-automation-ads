...
// account matching this app's naming pattern, so orphaned empty test
// campaigns from debugging/failed launches can be found and removed
// reliably instead of guessing IDs from log text.
...
Deno.serve(async (req: Request) => {
  ...
  const campaigns = (listData?.data || []).filter((c: any) => c.name?.startsWith("Yorvix") || c.name?.includes("CAPABILITY-TEST"));
